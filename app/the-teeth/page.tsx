...
export default function TheTeethPage() {
  ...
  return (
    <>
      ...
      {/* Navigation */}
      <nav className="sticky top-0 z-50 bg-[#080d1a]/95 backdrop-blur-md border-b border-[#3ddc84]/20 px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href="/" className="font-condensed font-bold text-xl text-white tracking-wide">
            PHIERS.org
          </Link>
          <div className="hidden md:block text-xs text-[#3ddc84]">
            PHIERS · Power Held In Every Representative's Seat
          </div>
        </div>
      ...
      <main>
        ...
        {/* ONION RINGS DIAGRAM */}
        <div className="bg-[#080d1a] py-12 px-4 text-center">
          ...
          <div className="relative w-[min(520px,92vw)] aspect-square mx-auto mb-8">
            ...
            <button
              ...
              onMouseEnter={() => {}}
              className={`absolute inset-0 rounded-full border-2 border-red-500/70 bg-gradient-radial from-red-500/0 via-red-500/0 to-red-500/18 shadow-[0_0_40px_rgba(230,57,70,0.25)] transition-all duration-300 hover:scale-105 ${activeRing === 0 ? 'scale-105 brightness-125' : ''}`}
              style={{ animation: 'firePulse 2.5s ease-in-out infinite' }}
            />
            {/* Ring 1 */}
            <button
              onClick={() => setActiveRing(activeRing === 1 ? null : 1)}
              className={`absolute inset-[10%] rounded-full border-2 border-red-500/50 bg-gradient-radial from-red-500/0 to-red-500/12 transition-all duration-300 hover:scale-105 ${activeRing === 1 ? 'scale-105 brightness-125' : ''}`}
            />
            {/* Ring 2 */}
            <button
              onClick={() => setActiveRing(activeRing === 2 ? null : 2)}
              className={`absolute inset-[22%] rounded-full border-2 border-yellow-500/50 bg-gradient-radial from-yellow-500/0 to-yellow-500/10 transition-all duration-300 hover:scale-105 ${activeRing === 2 ? 'scale-105 brightness-125' : ''}`}
            />
            {/* Ring 3 */}
            <button
              onClick={() => setActiveRing(activeRing === 3 ? null : 3)}
              className={`absolute inset-[34%] rounded-full border-2 border-[#3ddc84]/50 bg-gradient-radial from-[#3ddc84]/0 to-[#3ddc84]/10 transition-all duration-300 hover:scale-105 ${activeRing === 3 ? 'scale-105 brightness-125' : ''}`}
            />
            {/* Ring 4 */}
            <button
              onClick={() => setActiveRing(activeRing === 4 ? null : 4)}
              className={`absolute inset-[46%] rounded-full border-2 border-[#3ddc84]/70 bg-gradient-radial from-[#3ddc84]/5 to-[#3ddc84]/18 transition-all duration-300 hover:scale-105 ${activeRing === 4 ? 'scale-105 brightness-125' : ''}`}
            />
            {/* Center */}
            <button
          ...
          {/* Detail Panel */}
          <motion.div 
            className="w-[min(600px,92vw)] min-h-[140px] bg-[#0d1525]/95 rounded-xl p-6 mx-auto border-l-4"
        ...
        {/* THE TRUTH */}
        <section className="py-16 px-6 bg-[#0a1628] border-y border-[#3ddc84]/10">
        ...
        <section className="py-8 px-6 text-center">
          <div className="max-w-[600px] mx-auto">
            <Image 
              ...
              width={500}
              height={300}
              className="w-full max-w-[500px] h-auto mx-auto opacity-90"
            />
          </div>
        </section>
        ...
        <section className="py-16 px-6 max-w-[760px] mx-auto">
          ...
          <div className="space-y-6">
            {[
              { num: '1', title: 'You add your name', desc: 'Not symbolic. Not a petition that disappears. It\'s how real people are counted — by district.' },
              { num: '2', title: 'We organize by district', desc: 'Your name becomes part of a coordinated pressure block tied directly to your representative — based on what people in your district actually want.' },
              { num: '3', title: 'When enough people are organized', desc: 'The pressure becomes public and visible inside the district — creating a moment where your representative has to respond, in front of the people they represent.' },
              { num: '4', title: 'And when leaders ignore it', desc: 'They either respond — or they have to explain why they won\'t, publicly, on the record.', isRed: true },
            ].map((step, i) => (
          ...
          <div className="text-center mt-8">
            <Link href="/petition" className="inline-block bg-transparent text-[#3ddc84] border border-[#3ddc84]/50 px-8 py-3 rounded-full font-condensed font-bold hover:bg-[#3ddc84]/10 transition-all">
              ✍ Add Your Name →
            </Link>
        ...
        <section className="py-12 px-6 max-w-[800px] mx-auto">
          <div className="text-center mb-6">
            ...
            <div className="mt-6">
              <Link href="/petition" className="inline-block bg-transparent text-[#3ddc84] border border-[#3ddc84]/50 px-8 py-3 rounded-full font-condensed font-bold hover:bg-[#3ddc84]/10 transition-all">
                ✍ Add Your Name — Help Make Congress Do Its Job
              </Link>
            </div>
        ...
        <section className="py-16 px-6 bg-[#0a1628] border-y border-[#3ddc84]/10">
          <div className="max-w-[760px] mx-auto text-center">
            ...
            <div className="max-w-[380px] mx-auto mt-8">
              <button onClick={() => setChenowethModalOpen(true)} className="cursor-pointer">
                <Image 
                  src="/images/3.5pct_Erica_Chenoweth.jpg"
                  alt="3.5% — Chenoweth Research"
                  width={380}
                  height={250}
                  className="w-full h-auto rounded-lg border border-[#3ddc84]/20 hover:scale-[1.02] transition-transform"
                />
              </button>
        ...
        <section className="py-16 px-6 text-center">
          <div className="max-w-[560px] mx-auto">
            ...
            <Link href="/the-teeth" className="inline-block bg-transparent text-[#3ddc84] border border-[#3ddc84]/50 px-9 py-4 rounded-full font-condensed font-bold text-lg hover:bg-[#3ddc84]/10 transition-all tracking-wide">
              See What Makes This Unavoidable →
            </Link>
          </div>
        </section>
      ...
      {/* FOOTER */}
      <footer className="text-center py-12 px-6 border-t border-[#3ddc84]/10">
        <Image src="/images/PHIERS_Logo.png" alt="PHIERS.org" width={80} height={80} className="mx-auto mb-4 opacity-70" />
        <div className="font-condensed font-bold text-gray-400 mb-1">PHIERS is the system that gives every union — and everyone without one — the leverage to win.<br />Owned by the people. Built by the people.</div>
        <div className="text-xs text-gray-600 mt-6 leading-relaxed">
        ...
        {willModalOpen && (
          ...
          >
            <div className="relative max-w-[720px] w-full bg-[#080d1a]/97 border-2 border-[#3ddc84]/35 rounded-2xl shadow-2xl overflow-y-auto max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
              <button 
                onClick={() => setWillModalOpen(false)}
                className="absolute top-3 right-3 w-10 h-10 rounded-full bg-[#111d35] text-[#3ddc84] border-2 border-[#3ddc84] flex items-center justify-center text-xl hover:bg-[#3ddc84] hover:text-[#111d35] transition-all z-10"
              >
                ✕
              </button>
